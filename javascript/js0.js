...
function check_transport_area() {
	...
	if (g('sel_transport_area_id').value == '0') {
		showAlert('请选择物流配送地区', pNode, 'sel_transport_area_id_empty');
		return false;
	}
...
function check_pwd1_userreg() {
	...
	if (g('txtUserPwd').value.length < 6) {
		showAlert('密码需要大于等于6位！', pNode, 'txtUserPwd_empty');
		return false;
	}
...
function check_username_userreg() {
	...
	if (g('txtUserName').value.length < 6) {
		showAlert('电子邮件需要大于等于6位！', pNode, 'txtUserName_empty');
		return false;
	}
...
function sel_transport_area(area_id, basic_weight, fixed_postage, per_weight,
	...
	document.getElementById("ord_transportfund").innerHTML = "<font color=red><b>￥"
			+ transportprice_tag
			+ "元</b></font>&nbsp;&nbsp;&nbsp;订单商品总重量："
			+ profullweigth_tag / 1000 + "公斤 &nbsp;如对运费有异议，请确认订单后联系我们";
	if (document.getElementById("user_point_check").checked) {
		var p_userponit = parseInt(document.getElementById("user_point_hd").value);
		document.getElementById("ordertj").innerHTML = "<b>商品价格:</b><font color=red><b>￥"
				+ profullprice_tag
				+ "</b></font> + <b>订单运费:</b><font color=red><b>￥"
				+ transportprice_tag
				+ "</b></font> - <b>会员积分：</b><font color=green><b>"
				+ p_userponit
...
function isMoney(s) {
	 ...
	 * (re.test(s)) { return true; } else { window.alert ("请输入有效的金额，如23.78！");
	 * return false; }
	 */
...
function isMoney0(s, msg) {
	...
	if (isNaN(s)) {
		window.alert(msg + "请输入有效的金额，如23.78！");
		return false;
	} else {
		return true;
	}
	 ...
	 * (msg+"请输入有效的金额，如23.78！"); return false; }
	 */
}
...
function isWeight(s) {
	...
	/*
	 * var regu = "^[0-9]+[\.][0-9]{0,2}$"; var re = new RegExp(regu); if
	 * (re.test(s)) { return true; } else { window.alert ("请输入有效的重量，如23.78！");
	 * return false; }
	 */
